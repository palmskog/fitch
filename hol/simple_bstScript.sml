...
open comparisonTheory finite_mapTheory pred_setTheory;
...
Theorem key_set_equiv:
   !cmp. good_cmp cmp ==>
    (!k. k IN key_set cmp k) /\
    (!k1 k2. k1 IN key_set cmp k2 ==> k2 IN key_set cmp k1) ∧
    (!k1 k2 k3. k1 IN key_set cmp k2 /\ k2 IN key_set cmp k3 ==> k1 IN key_set cmp k3)
Proof
  rw [key_set_def] >>
...
val IN_key_set = save_thm("IN_key_set",
  ``k' IN key_set cmp k`` |> SIMP_CONV (srw_ss()) [key_set_def]);
...
Theorem to_fmap_key_set:
   !ks t. ks IN FDOM (to_fmap cmp t) ⇒ ?k. ks = key_set cmp k
Proof
   Induct_on `t` >>
   rw[to_fmap_def] \\
   metis_tac[]
...
Theorem key_ordered_to_fmap:
   ...
   !t k res. key_ordered cmp k t res <=>
       (!ks k'. ks IN FDOM (to_fmap cmp t) /\ k' IN ks ==> cmp k k' = res)
Proof
  strip_tac \\
  Induct \\
  rw[to_fmap_def] \\
...
Theorem wf_tree_Node_imp:
   ...
   (!x. key_set cmp x IN FDOM (to_fmap cmp l) ==> cmp k x = Greater) /\
   (!x. key_set cmp x IN FDOM (to_fmap cmp r) ==> cmp k x = Less)
Proof
